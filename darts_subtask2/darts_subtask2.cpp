...
void detectAndDisplay( Mat frame, string imageName)
{
	...
	{
		// 4. Draw box around darts found ground truth
	...
	for( int i = 0; i < groundTruthv.size(); i++ )
	{
		for( int k = 0; k < darts.size(); k++ )
		{
			...
			double iouValue= iou(darts[k].x, darts[k].y, (darts[k].x + darts[k].width),(darts[k].y+ darts[k].height),groundTruthv[i][j], groundTruthv[i][j+1], groundTruthv[i][j+2],groundTruthv[i][j+3]);
	...
	falsePositives = darts.size() - truePositives;
	...
	//Results
	std::cout << "Darts Detected: "<< darts.size() << std::endl;
